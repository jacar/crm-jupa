...
export class PdfService {
  ...
  async generateQuotePdfBuffer(quote: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.fillQuoteDoc(doc, quote);
        doc.end();
      } catch (error) {
        this.logger.error('Error generating PDF buffer', error);
        reject(error);
      }
    });
  }
  ...
  private fillQuoteDoc(doc: any, quote: any) {
    // Header
